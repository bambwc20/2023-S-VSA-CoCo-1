...
export async function FilterDialogues(list_id, user_id) {
  ...
  const filterFirstStep = firstStep.map((firstStep) => {
    const isBookmarked = bookmark.some(
      (bookmark) => bookmark.conversation_id === firstStep.id
    );
    return {
      id: firstStep.id,
...
export async function FilterNurse(list_id, user_id) {
  ...
  const filterSecondStep = secondStep.map((secondStep) => {
    if (secondStep.speaker === "Nurse" || secondStep.speaker === "nurse") {
      const isBookmarked = bookmark.some(
        (bookmark) => bookmark.conversation_id === secondStep.id
      );
      return {
        id: secondStep.id,
...
export async function AccuracyStep2(id, reply) {
  // 문장별 id와 reply(클라이언트에서 받은 사용자의 대답)
...
export async function AccuracyStep3(id, reply) {
  // 문장별 id와 reply(클라이언트에서 받은 사용자의 대답)
...
export async function FilterNurse_ThirdStep(list_id, user_id) {
  ...
  const filterThirdStep = thirdStep.map((thirdStep) => {
    if (thirdStep.speaker === "Nurse" || thirdStep.speaker === "nurse") {
      // speaker가 간호사일 경우 대화 비움
      const isBookmarked = bookmark.some(
        (bookmark) => bookmark.conversation_id === thirdStep.id
      );
      return {
        id: thirdStep.id,
      ...
      // speaker가 간호사가 아닐경우(환자) 대화내용 그대로
      return {
        id: thirdStep.id,
        speaker: thirdStep.speaker,
        dialogue: thirdStep.dialogue,
