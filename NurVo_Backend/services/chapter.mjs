...
export async function ChapterByTopic(id) {
  ...
  const mergedChapter = chapter.map((data) => {
    // 전체 chapter에 대해
    const matchingData = chapterWithStep.find(
      (newData) => newData.id == data.id
    ); // 학습기록이 있는 chapter의 id와 일치하는 id 찾음
    return {
      ...data,
  ...
  const result = [];
  topic.forEach((topic) => {
    const chapterByTopic = mergedChapter.filter(
      (chapter) => chapter.topic_id === topic.id
    );
