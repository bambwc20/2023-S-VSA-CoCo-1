...
//아이디로 챕터에 대한 설명 요청ㄱ
export async function fetchChapterDescriptionById(
  chapterId: string,
...
export async function fetchChapterDialogueById(
  chapterId: number,
): Promise<ResponseProps | undefined> {
  const url = `${HOST_URL}/api/dialogues/${chapterId}`;
  const USER_TOKEN = await retrieveUserSession();
...
export async function fetchChapterDialogueSecondStepById(
  chapterId: number,
): Promise<ResponseProps | undefined> {
...
export async function fetchChapterDialogueThirdStepById(
  chapterId: number,
): Promise<ResponseProps | undefined> {
...
//2단계 정확도 계산(post)
export async function calculateSecondStepAccuracyWithSentenceId(
  chapterId: number,
  sentenceId: string,
  reply: string,
): Promise<ResponseProps | undefined> {
...
//3단계 정확도 계산(post)
export async function calculateThirdStepAccuracyWithSentenceId(
  chapterId: number,
  sentenceId: string,
  reply: string,
): Promise<ResponseProps | undefined> {
...
//문장 북마크 저장
export async function addSentenceBookmark(
  sentenceId: number,
): Promise<ResponseProps | undefined> {
...
//문장 북마크 삭제
export async function deleteSentenceBookmark(
  sentenceId: number,
): Promise<ResponseProps | undefined> {
