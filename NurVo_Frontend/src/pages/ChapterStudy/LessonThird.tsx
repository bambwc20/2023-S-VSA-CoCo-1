...
import {
  calculateThirdStepAccuracyWithSentenceId,
  completeChapter,
  fetchChapterDialogueThirdStepById,
...
export default function LessonThird({navigation, route}: LessonThirdProps) {
      ...
      if (messages.length < allMessages.length) {
        if (
          messages[messages.length - 1].speaker.trim().toLowerCase() ===
            'nurse' &&
          messages[messages.length - 1].dialogue
        ) {
  ...
  async function calculateCorrectPercent() {
    const result = await calculateThirdStepAccuracyWithSentenceId(
        ...
        renderItem={({item, index}) => (
          <TouchableOpacity onPress={handlePress} activeOpacity={1}>
            <ChatBubbleInputAll
              ...
              onIsClickSpeakChange={(isSpeaking: boolean) =>
