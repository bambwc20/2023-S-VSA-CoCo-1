...
export default function LessonFirst({navigation, route}: LessonFirstProps) {
  ...
  const {allMessages, messages, showAlert, isSpeaking} = state;
    ...
    if (allMessages.length > 0) {
      ...
      if (isSoundMode) {
        ...
        speech(
          ...
          () => setIsSpeakingByIndex(0, false),
        ); //speech함수가 끝나면 setIsSpeaking(false)로 바꿔줌
      }
    }
  }, [allMessages]);
      ...
      if (messages.length < allMessages.length) {
        ...
        if (isSoundMode) {
          ...
          speech(
            ...
            allMessages[messages.length].speaker.trim().toLowerCase() ===
              'nurse',
            () => setIsSpeakingByIndex(messages.length, false),
          ); //speech함수가 끝나면 setIsSpeaking(false)로 바꿔줌
        }
      } else {
  ...
  const handleNext = () => {
    completeChapter(
      ...
      route.params.step > 1 ? route.params.step : 1,
    );
    navigation.pop();
